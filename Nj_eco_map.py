...
# eBird key — get a free one at https://ebird.org/api/keygen
# Set via env var so secrets aren't committed:  export EBIRD_API_KEY=...
...
# 10 ≈ NJ statewide median; tuned to be neutral for unknown areas.
UNKNOWN_LP_DEFAULT = 10.0
...
def compute_danger_scores():
    ...
    Returns list of (species, score, mean_lp, data_dict) tuples, sorted by score descending.
    """
    results = []
    for sp, d in SPECIES_DANGER.items():
        counties = d["lp_counties"]
        mean_lp  = sum(NJ_LP_DATA.get(co, UNKNOWN_LP_DEFAULT) for co in counties) / len(counties)
...
def main() -> None:
    ...
    if EBIRD_API_KEY:
        ...
        print(
            ...
            "     Set the EBIRD_API_KEY environment variable to enable.\n"
    ...
    # Try a few ports in case the default is already in use; fall back to OS-assigned.
    httpd = None
    bound_port = None
    for candidate in (SERVE_PORT, SERVE_PORT + 1, SERVE_PORT + 2, 0):
        try:
            httpd = socketserver.TCPServer(("", candidate), _H)
            bound_port = httpd.server_address[1]
            break
        except OSError as exc:
            print(f"  ⚠  Port {candidate} unavailable ({exc}); trying next…")

    if httpd is None:
        print(f"  ✗ Could not bind a local server port. Open the file directly: {abs_path}")
        return
    ...
    url = f"http://localhost:{bound_port}/{OUTPUT_FILE}"
